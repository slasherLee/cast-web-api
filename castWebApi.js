					...
					if (path[2] == 'disconnected') {
						...
						.then(device => {
							if (path[3]) {
								...
								if (path[3]=='startTbsRadio') {
                                    try {
									    result = getDevice(path[2]).startTbsRadio();
									} catch (e) {
										result = {response:'error', error: e};
									}
                                }
								if (path[3]=='startSleepMp3') {
                                    try {
									    result = getDevice(path[2]).startSleepMp3(res);
									} catch (e) {
										result = {response:'error', error: e};
									}
                                }
...
function CastDevice(id, address, name) {
    ...
    this.startTbsRadio = function() {
		log('info', 'CastDevice.startTbsRadio()', '', this.id);
        startTbsRadio();
		return {response:'ok'};
    }
    this.startSleepMp3 = function(res) {
		log('info', 'CastDevice.startSleepMp3()', '', this.id);
        startSleepMp3(res)
		return {response:'ok'};
    } 
...
function replaceGoogleTts(mediaList) {
	return new Promise( function(resolve, reject) {
		...
		mediaList.forEach(function(mediaElement){
			if (mediaElement.googleTTS && mediaElement.googleTTS!=null) {
				googleTTS(decodeURI(mediaElement.media.metadata.title), mediaElement.googleTTS, 1)
				.then(function (url) {
...
function startSleepMp3(res) {
    var fs = require('fs')
    var file = fs.createReadStream("../media/delta_wave_for_deep_sleep.mp3");
    file.pipe(res);
}
