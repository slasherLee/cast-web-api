...
def setMediaPlaybackShortOther(mediaType, provider, phrase, mediaStreamType, mediaTitle, mediaSubtitle, mediaImageUrl) {
    ...
    if( provider == "Naver" || provider == "Google" ) {
    	sendHttpRequest(getDataValue('apiHost'), '/setMediaPlayback' + provider + '?address='+getDataValue('deviceAddress')+'&mediaType='+mediaType+'&ttsText='+urlEncode(phrase)+'&mediaStreamType='+mediaStreamType+'&mediaTitle='+urlEncode(mediaTitle)+'&mediaSubtitle='+urlEncode(mediaSubtitle)+'&mediaImageUrl='+mediaImageUrl)
    }
}
